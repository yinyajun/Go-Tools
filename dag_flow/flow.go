...
func (df *DagFlow) Graph() *AcyclicGraph {
	return &df.acyclicGraph
}
...
func (df *DagFlow) Connect(from, to JobNode) {
	...
	df.acyclicGraph.Connect(BasicEdge(from, to))
}
...
func (rm *RootsMap) Add(jobNode JobNode) {
	...
	rm.rootsMap[Hashable(jobNode)] = jobNode
}
